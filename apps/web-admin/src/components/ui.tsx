...
// Small shared primitives — token classes only (ADR-009 brand-agnostic).
...
/** Signed Toman amount — negative renders in danger. */
export function Toman({ rial, className = '' }: { rial: string; className?: string }) {
  const negative = rial.startsWith('-');
  return (
    <span
      className={`whitespace-nowrap tabular-nums ${negative ? 'font-medium text-danger' : ''} ${className}`}
    >
      {formatToman(rial)} <span className="text-xs text-fg-muted">تومان</span>
    </span>
  );
}
...
export function ErrorState({ error, onRetry }: { error: unknown; onRetry?: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 rounded-md border border-border bg-surface px-6 py-10 text-center shadow-card">
      ...
      {detail ? <p className="max-w-md text-xs text-fg-muted">{detail}</p> : null}
      {onRetry ? (
        <button
          type="button"
          onClick={onRetry}
          className="mt-2 rounded-sm border border-border bg-surface px-3 py-1.5 text-sm hover:bg-surface-2"
        >
          تلاش دوباره
        </button>
      ) : null}
