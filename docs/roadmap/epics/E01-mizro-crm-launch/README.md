...
- 🔒 **«امروز من» now covers lead-less accounts.** The ＋'s «مشتری» files a customer with no pipeline entry, so a visit logged against it carries its next action on the *activity* row — and `GET /v1/activities/today` read the day from leads alone, dropping exactly those. The seller's day is now the union, with the latest activity per account winning (a newer visit supersedes what the previous one promised). Without this, "nothing rots" held for leads and quietly failed for every account that never was one.
